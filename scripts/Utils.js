import { ATTACKTYPES, ATTACK_DATA_TEMPLATE } from "./Settings";
...
async function getIndex({ name = "", itemId = "" }) {
    var itemPacks = game.packs.filter((f) => f.metadata.entity === "Item")
    .map((m) => {
...
export async function GetItemData(
  ...
  if (!getItem) itemData = await getIndex({ itemId: itemId });
  else itemData = await getIndex({ name: getItem.data.name });
...
export function resetDamageIfAreaEffect( attackData, isAreaEffect = false) {
  if (isAreaEffect) attackData.damageTotal = 0;
  return attackData;
}
...
export async function CombatantStats(combatantStat) {
  let damageTotalArray = combatantStat.events
    .filter((f) => {
      return isValidAtttack(f.actionType);
    })
    .map((m) => {
...
let groupBy = function (ex, key) {
  return XSLTProcessor.reduce(function (rv, x) {
